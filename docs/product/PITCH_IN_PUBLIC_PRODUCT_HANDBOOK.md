...
| Event pitch privacy | Shipped | Pitches submitted to an event are private by default (`pitches.event_id` + `visibility`), enforced by RLS with `can_view_pitch` / `can_view_pitch_via_event_submission`. Joining an event requires an invitation or access code at every visibility. Prod b7c5a66 / be86b08. |
| Founder-controlled promotion to the public feed | Shipped | Owner-only `POST /api/pitches/[pitchId]/visibility` moves one take between private and the public feed. This is the intended path for sharing a best take. Prod b7c5a66. |
| Event cohort feed | Shipped | `GET /api/pitches?eventSlug=` scopes the feed to one event and lets RLS decide membership. Prod be86b08. |
| Cohort peer feedback | Shipped | Any ACTIVE participant — including a peer founder — may give feedback on a cohort take. Authorised by membership, not by a pre-built review assignment. `pitch_events.peer_feedback_enabled` (default true) lets an organizer close it for a competition; archived events are always closed. Peer reviews are tagged `cohort_peer_feedback` and EXCLUDED from organizer review coverage, reported beside it as `peerReviewsCompleted`. Prod 52f8c86. |
| Founder navigation shell | Shipped | Close (X) control on every action page; routed bottom tab bar on `/events`, `/events/[slug]`, `/profile/[userId]`, `/pitch/[id]` including their loading and error branches; pending-invitation badge on routed pages, announced via the accessible name. Prod 86c8d8c / b3ec551. |
| Founder deck visibility | Shipped | A founder sees and opens their own deck from their profile (`GET /api/startup/deck/view`, owner-only, company resolved from the session, `private, no-store`). `?profileEdit=1` deep-links to the deck field. Prod 86c8d8c / b3ec551. |
| Video upload ownership | Shipped | `video_uploads` binds every Cloudflare video id to its uploader at the moment the upload URL is issued. Required to read a video's status and to attach it to a pitch. Service-write only, RLS read-by-owner, backfilled from existing pitches. Prod 7ab2b1e. |
| Signed video playback (private pitches) | Shipped | Private pitches receive short-lived signed playback and thumbnail URLs, minted after RLS has selected the rows so the token inherits the row's visibility. Public pitches deliberately keep a permanent canonical URL so a founder can share a best take on social media. Provider ids are withheld on private pitches so the canonical URL cannot be reconstructed. Prod 7ab2b1e / 9a6829a / 4ed0f47. |
| Cloudflare `requireSignedURLs` enforcement | Planned | The remaining piece of the replay gap: a URL captured before a pitch went private still plays, because the videos themselves are not enforced. Binding enforcement to the API endpoints that change visibility is NOT sound — the RLS UPDATE policy on `pitches` has `with_check: NULL`, so an owner can change visibility directly through PostgREST. Requires a database trigger plus an outbox and a worker. See `.ship-loop/backlog.md` for the measured residual risk. |
