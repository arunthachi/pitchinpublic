...
export async function POST(request: NextRequest) {
  try {
    ...
    const subject = `[Pitch in Public] ${lead.type === 'founder' ? 'Founder invite request' : 'Organizer invite request'}: ${lead.name}`;
    const rows = [
      ['Type', lead.type],
      ['Email', lead.email],
    ...
    const html = `
      <div style="font-family: Inter, Arial, sans-serif; background:#050608; color:#f8fafc; padding:24px;">
        <div style="max-width:640px; margin:0 auto; border:1px solid rgba(255,255,255,.14); border-radius:24px; padding:24px; background:#0f172a;">
          ...
          <h1 style="margin:8px 0 20px; font-size:28px;">${escapeHtml(lead.type === 'founder' ? 'Founder invite request' : 'Organizer invite request')}</h1>
