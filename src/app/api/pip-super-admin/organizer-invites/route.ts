...
export async function POST(request: NextRequest) {
  ...
  if (payload.sendEmail) {
    const emailResult = await sendEmail({
      ...
      subject: `Your organizer access is approved for ${payload.organizationName}`,
      text: [
        `Your Pitch in Public organizer access is approved for ${payload.organizationName}.`,
        '',
        'Next steps:',
      ...
      html: `
        <div style="font-family: Inter, Arial, sans-serif; background:#050608; color:#f8fafc; padding:24px;">
          <div style="max-width:640px; margin:0 auto; border:1px solid rgba(255,255,255,.14); border-radius:24px; padding:24px; background:#0f172a;">
            ...
            <h1 style="margin:8px 0 16px; font-size:28px;">You are invited to organize ${escapeHtml(payload.organizationName)} on Pitch in Public</h1>
