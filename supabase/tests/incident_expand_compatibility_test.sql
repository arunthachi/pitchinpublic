...
SELECT ok(
  ...
  'PR2 removes the temporary feedback identity read after cutover'
);
SELECT ok(
  ...
  AND NOT has_column_privilege('authenticated', 'public.feedback', 'disclosure_mode', 'SELECT'),
  'PR2 retains structured feedback reads without disclosure metadata'
);
SELECT ok(
  NOT has_table_privilege('authenticated', 'public.review_assignments', 'SELECT'),
  'PR2 removes direct assignment reads after the application cutover'
);
SELECT ok(
...
SELECT ok(
  ...
  'compatible application assignment projections remain available after PR2'
);
SELECT ok(
  ...
  'compatible application feedback projections remain available after PR2'
);
SELECT ok(
