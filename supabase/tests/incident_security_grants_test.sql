...
SELECT ok(
  ...
  'PR2 removes direct reviewer identity reads after cutover'
);
SELECT ok(
  NOT has_column_privilege('authenticated', 'public.feedback', 'reviewer_role', 'SELECT'),
  'PR2 removes direct reviewer role reads after cutover'
);
SELECT ok(
  NOT has_column_privilege('authenticated', 'public.feedback', 'disclosure_mode', 'SELECT'),
  'PR2 removes direct disclosure-mode reads after cutover'
);
SELECT ok(
